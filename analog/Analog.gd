...
func _ready():
	...
	parent = get_parent();
	halfSize = bg.texture.get_size() /3.5; #aqui e onde edita ate onde o ponteiro do mouse pode buxar a bolinha troque o 4
...
func _input(event):
	...
	if need2ChangeActivePointer(event):
		if (currentPointerIDX != incomingPointer) and event.is_pressed():
			...
			showAtPos(Vector2(event.position.x, 0));
...
func process_input(event):
	calculateForce(event.position.x - self.get_global_position().x, 0 - self.get_global_position().x)
	updateBallPos()
...
func sendSignal2Listener():
	if (_listenerNodePath != null):
		_listenerNodePath.controle_analogico(currentForce,_name) #self)
